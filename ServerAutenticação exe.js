...
let Estudantes = [
  { id: 1, nome: "Leonardo", idade: "18", curso: "T.I" },
  { id: 2, nome: "Ruan", idade: "18", curso: "T.I" },
  { id: 3, nome: "Eric", idade: "18", curso: "T.I" },
];
...
app.get('/Estudantes/listarEstudantes/:id', authenticate, (req, res) => {
  ...
  const servidor = Estudantes.find(s => s.id === id); // Encontra o servidor com o ID especificado
  if (servidor) {
...
app.post('/Estudantes/inserir', authenticate, (req, res) => {
  ...
  novoServidor.id = Estudantes.length ? Estudantes[Estudantes.length - 1].id + 1 : 1; // Define o ID do novo servidor
  Estudantes.push(novoServidor); // Adiciona o novo servidor ao array
